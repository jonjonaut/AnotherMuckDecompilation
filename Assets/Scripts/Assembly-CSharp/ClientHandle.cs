...
public class ClientHandle : MonoBehaviour
{
	...
	public static void FinalizeBuild(Packet packet)
	{
		...
		MonoBehaviour.print($"Received build #{num2}, now building {ItemManager.Instance.allItems[num].name} @ {vector} (rotated {num3})");
		BuildManager.Instance.BuildItem(buildOwner, num, num2, vector, num3);
	}
	...
	{
		int num = packet.ReadInt();
		GameManager.instance.UpdateDay(num);
		DayCycle.totalTime = num;
	}
